...
contract('3IE.IO Contract', function (accounts) {
  ...
  describe('deploy hackathon contract', async function () {
    ...
    it('create bounty', async () => {
        ...
        assert.equal(balance, bountyContribution, "bounty balance doesn't match value sent");
        assert.equal(payout, bountyPayout, "bounty payout doesn't match argument passed");
        assert.equal(minimumRep, bountyMinimuimRep, "bounty minRep doesn't match argument passed");
    });
  });
 });